...
public class Elevator extends SubsystemBase {
    ...
    public Command sysIdRoutine() {
        return (sysIdQuasistatic(SysIdRoutine.Direction.kForward).until(() -> inputs.elevatorPos > .45))
                ...
                .andThen(sysIdQuasistatic(SysIdRoutine.Direction.kReverse).until(() -> inputs.elevatorPos < .15))
                .andThen(holdPosition().withTimeout(0.0))
                .andThen(sysIdDynamic(SysIdRoutine.Direction.kForward).until(() -> inputs.elevatorPos > .45))
                .andThen(holdPosition().withTimeout(0.0))
                .andThen(sysIdDynamic(SysIdRoutine.Direction.kReverse).until(() -> inputs.elevatorPos < .15))
    ...
    public Elevator(ElevatorIO io) {
        ...
        if (Robot.isSimulation()) {
            feedforward = new ElevatorFeedforward(
                    ElevatorConstants.simKs, ElevatorConstants.simKg, ElevatorConstants.simKv, ElevatorConstants.simKa);
        } else {
            feedforward = new ElevatorFeedforward(
    ...
    protected Command moveToSetpoint(double position, String name) {
        return requestElevatorPosition(position).withName("Move to pos: " + name);
    }

    protected Command moveToStore(BooleanSupplier hasAlgae) {
        return either(
                        moveToSetpoint(StorePositions.storeAlgae.elevator, "algae store"),
                        moveToSetpoint(StorePositions.storeCoral.elevator, "coral store"),
                        hasAlgae)
    ...
    private boolean isAtSetpoint() {
        if (Math.abs(inputs.elevatorPos - inputs.finalSetpoint) < ElevatorConstants.allowedClosedLoopError) {
            return true;
        }
        return false;
    }
